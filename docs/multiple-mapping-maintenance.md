...
| POST | `/Form/FormMultipleMapping/sequence/reorder` | 依 orderedSids 重新編排指定 Base 範圍內 Mapping 資料列的 SEQ。 | 需帶入 `formMasterId`、`orderedSids` 與 `scope.basePkValue`，成功時回傳更新筆數。 |
...
1. **欄位與資料表檢查**
   ...
   - 執行操作時再次驗證欄位名稱格式（僅允許英數與底線）與欄位存在性，避免錯誤設定或 SQL Injection。 【F:Areas/Form/Services/FormMultipleMappingService.cs†L221-L255】【F:Areas/Form/Services/FormMultipleMappingService.cs†L353-L370】
...
3. **左右清單產出**
   - 左側（已關聯）：依 Mapping.BaseFK = BaseId 取得 Detail FK 清單，再以 `IN` 撈出 Detail 全欄位資料並包裝為 `MultipleMappingItemViewModel`。 【F:Areas/Form/Services/FormMultipleMappingService.cs†L72-L105】【F:Areas/Form/Services/FormMultipleMappingService.cs†L258-L268】
   - 右側（未關聯）：以 `NOT EXISTS` 方式排除已關聯資料，確保與左側互斥。 【F:Areas/Form/Services/FormMultipleMappingService.cs†L270-L283】

4. **取得 Base 清單**
   - `/search` 端點呼叫 `FormMultipleMappingService.GetForms`，以 `FormFunctionType.MultipleMappingMaintenance` 為固定功能類型委派到共用的 `FormService.GetFormList`，確保回傳欄位與資料行為與其他表單一致。 【F:Areas/Form/Controllers/FormMultipleMappingController.cs†L37-L56】【F:Areas/Form/Services/FormMultipleMappingService.cs†L64-L69】

5. **批次新增/移除**
   - 透過 `ITransactionService` 保證同一批次在單一交易內完成。
   - 新增時使用 `IF NOT EXISTS ... INSERT` 避免重複關聯；移除時逐筆 `DELETE`，均會先確認 Base/Detail 資料存在。 【F:Areas/Form/Services/FormMultipleMappingService.cs†L108-L168】

6. **Mapping.SEQ 重新排序**
   - 端點 `/sequence/reorder` 會驗證 orderedSids 不為空、不可重複，且清單筆數須與同一 Base 範圍的 Mapping 資料列數一致，避免產生殘留 SEQ。 【F:Areas/Form/Services/FormMultipleMappingService.cs†L170-L219】【F:Areas/Form/Services/FormMultipleMappingService.cs†L309-L412】
   - 交易內以 `VALUES` 建立的 set-based CTE（等效 TVP）更新 Mapping.SEQ，避免逐筆更新並確保單一 Commit。 【F:Areas/Form/Services/FormMultipleMappingService.cs†L191-L218】

## 程式流程補充

1. 前端先呼叫 `/search`，帶入指定的 `FormMasterId` 與查詢條件取得 Base 清單，並從結果中的 `Pk` 取出目標主鍵值。
2. 前端將取得的 `Pk` 填入 `baseId`，呼叫 `/items` 相關端點取得左右清單或進行新增/移除。
3. 若需要重排 SEQ，將 `orderedSids` 依最終順序送入 `/sequence/reorder`，服務端會驗證 SID 存在與 Base 範圍，再一次性更新 SEQ。
4. 左右清單的計算與新增/移除流程如上節說明，均會在讀寫前做欄位與資料存在性檢查，並使用交易確保一致性。
...
- **排序清單缺漏**：重排時會比對 orderedSids 與指定 Base 的資料筆數，並確認每個 SID 皆隸屬於該 Base，避免 SEQ 留空或跨 Base 更新。
...
- 模型：`Areas/Form/ViewModels/MultipleMappingOperationViewModels.cs`、`Areas/Form/ViewModels/MultipleMappingReorderViewModel.cs`
