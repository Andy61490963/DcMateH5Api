...
public class CreateLotAsyncIntegrationTests
{
    [Fact]
    [Trait("Category", "Integration")]
    public async Task CreateLotAsync_ShouldInsertAndCleanupLotData()
    {
        var connectionString = TestConfiguration.LoadConnectionString();
        var arrangement = await LoadArrangementAsync(connectionString);
        var lotCode = $"ITEST-{DateTime.UtcNow:yyyyMMddHHmmssfff}";
        const decimal lotQty = 3m;
        ...
        try
        {
            var result = await service.CreateLotAsync(
                new WipCreateLotInputDto
                {
                    ...
                    COMMENT = "CreateLotAsync integration test"
                });
            ...
            Assert.NotNull(lot);
            Assert.Equal(arrangement.WorkOrder, lot!.WO);
            Assert.Equal(arrangement.PartNo, lot.PART_NO);
            Assert.Equal(lotQty, lot.LOT_QTY);
            Assert.Equal("Wait", lot.LOT_STATUS_CODE);
            Assert.Equal("N", lot.CUR_OPER_FIRST_IN_FLAG);

            var histActions = (await conn.QueryAsync<string>(
                """
                SELECT ACTION_CODE
                FROM WIP_LOT_HIST
                WHERE LOT = @Lot
            ...
            Assert.Equal(2, histActions.Count);
            Assert.Equal("CREATE_LOT", histActions[0]);
            Assert.Equal("OPER_START", histActions[1]);

            var releaseQty = await conn.ExecuteScalarAsync<decimal?>(
                "SELECT RELEASE_QTY FROM WIP_WO WHERE WO = @Wo",
                new { Wo = arrangement.WorkOrder });

            Assert.Equal((arrangement.PreviousReleaseQty ?? 0) + lotQty, releaseQty ?? 0);
        }
        finally
        {
    ...
    private sealed class CreatedLotRow
    {
