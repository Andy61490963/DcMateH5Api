...
";
    ...
    public async Task<FormFieldListViewModel> GetFieldsByTableName( string tableName, Guid? formMasterId, TableSchemaQueryType schemaType )
    {
        ...
        foreach (var col in columns)
        {
            ...
            var vm = new FormFieldViewModel
            {
                ...
                DETAIL_TO_RELATION_DEFAULT_COLNUM = cfg?.DETAIL_TO_RELATION_DEFAULT_COLUMN
            };
    ...
    public void UpsertField(FormFieldViewModel model, Guid formMasterId)
    {
        ...
        var param = new
        {
            ...
            model.CAN_QUERY,
            model.DETAIL_TO_RELATION_DEFAULT_COLNUM
        };
...
WHEN MATCHED THEN
    UPDATE SET
        ...
        CAN_QUERY      = @CAN_QUERY,
        DETAIL_TO_RELATION_DEFAULT_COLNUM = @DETAIL_TO_RELATION_DEFAULT_COLNUM,
        EDIT_TIME      = GETDATE()
WHEN NOT MATCHED THEN
    INSERT (
        ID, FORM_FIELD_MASTER_ID, TABLE_NAME, COLUMN_NAME, DISPLAY_NAME, DATA_TYPE,
        CONTROL_TYPE, IS_REQUIRED, IS_EDITABLE, QUERY_DEFAULT_VALUE, FIELD_ORDER, QUERY_COMPONENT, QUERY_CONDITION, CAN_QUERY, DETAIL_TO_RELATION_DEFAULT_COLNUM, CREATE_TIME, IS_DELETE
    )
    VALUES (
        @ID, @FORM_FIELD_MASTER_ID, @TABLE_NAME, @COLUMN_NAME, @DISPLAY_NAME, @DATA_TYPE,
        @CONTROL_TYPE, @IS_REQUIRED, @IS_EDITABLE, @QUERY_DEFAULT_VALUE, @FIELD_ORDER, @QUERY_COMPONENT, @QUERY_CONDITION, @CAN_QUERY, @DETAIL_TO_RELATION_DEFAULT_COLNUM, GETDATE(), 0
    );";
